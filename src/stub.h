...
class Stub {
   public:
...
#else
            if (0 == mprotect(pageof(pstub->fn), m_pagesize * 2,
...
#ifdef _WIN32
                VirtualProtect(pageof(pstub->fn), m_pagesize * 2,
                               PAGE_EXECUTE_READ, &lpflOldProtect);
#else
                mprotect(pageof(pstub->fn), m_pagesize * 2,
                         PROT_READ | PROT_EXEC);
#endif
            }
    ...
    void set(T addr, S addr_stub) {
        ...
        if (0 == VirtualProtect(pageof(pstub->fn), m_pagesize * 2,
                                PAGE_EXECUTE_READWRITE, &lpflOldProtect))
#else
        if (-1 == mprotect(pageof(pstub->fn), m_pagesize * 2,
                           PROT_READ | PROT_WRITE | PROT_EXEC))
#endif
        {
   ...
   private:
    ...
    bool distanceof(char *addr, char *addr_stub) {
        std::ptrdiff_t diff =
            addr_stub >= addr ? addr_stub - addr : addr - addr_stub;
